...
// One Redis list per pool for searcher arb orders (ToBOrders), separate from the intent queue and the
// legacy bid queue. Unlike the intent/bid queues, orders are READ (not drained) by the seal endpoint:
// every operator must receive the identical sealed set for a block, so the relay serves the same bytes
// to all and lets `validForBlock` scope each order to exactly one block.
