...
describe("clearingPriceX128 (frozen §2.2b derivation)", () => {
    it("stamps currency1-raw per currency0-raw in Q128 for USDC(6)/WETH(18) @ 0.0005 WETH/USDC", () => {
        // 0.0005 WETH per USDC → 1 USDC-raw (1e-6 USDC) buys 5e8 WETH-raw → price = 5e8, scaled by 2^128.
        expect(clearingPriceX128(0.0005, 6, 18)).toBe(500_000_000n << 128n);
    });
...
describe("buildSealedSet wire shape", () => {
    ...
    it("encodes bigints as strings, blocks as numbers, and derives the reference block", () => {
        ...
        expect(sealed.referenceBlockNumber).toBe(98); // target - REF_OFFSET
        expect(sealed.clearingPriceX128).toBe((500_000_000n << 128n).toString());
